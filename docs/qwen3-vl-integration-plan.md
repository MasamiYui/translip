...
| 场景 | 消费方 | 增益 |
| --- | --- | --- |
| **V1 场景上下文** | translation 翻译 | 每个 ContextUnit 附带一句画面描述，减少称谓性别/指代/语气误译 |
| **V2 说话人视觉归属** | speaker-registry 角色库 | 判断画面人数/谁在说话，辅助声纹聚类；角色库获得"有脸的角色" |
...
| 提取的信息 | 用在哪 | 收益 |
| --- | --- | --- |
| 场景描述 + 人物关系 + 情绪 | translation 翻译 prompt | 称谓性别、指代、语气误译减少（你→tu/vous、兄/弟这类） |
| 画面人数 + 谁在说话 | speaker-registry 声纹聚类 | 同性别相近音色的说话人区分；角色库获得头像和画面出场记录 |
...
`auto` 解析顺序：macOS arm64 且 `mlx_vlm` 可导入 → mlx；否则探测 `OLLAMA_HOST`（默认 `http://127.0.0.1:11434`）可达且有 qwen3-vl 模型 → ollama；都没有 → 清晰的依赖错误（仿照 synthesis 对 moss-tts-nano 的处理）。
...
1. **segments 驱动**（管线内 / 提供了 `--segments`）：把 transcription 的 segments 按"同说话人 + 间隔 ≤ 阈值 + 时长上限"聚成区间。注意这是 vision 自己的聚合（参数独立配置），**不试图复刻 translation 的 `build_context_units` 产出同样的 unit_id** —— 消费侧用时间重叠匹配（§6.5），不依赖两边分组一致。
...
> `unit_id`（`vis-` 前缀）只是文件内局部 id，**不与 translation 的 ContextUnit id 对齐**——后者是 translation 运行时在自己进程内现场编号的（`unit-{序号}`），跨进程复刻同样的编号要求两边分组参数与输入 segments 文件永远一致，是巧合性正确。消费侧匹配协议见 §6.5：以**时间区间重叠**为准；segments 驱动时附带 `segment_ids`（segments JSON 中的稳定 id）作为精确索引，固定间隔模式下该字段为空数组。
...
- 依赖 transcription（需要 segments 的时间轴）。`sequence_hint=45` 落在 speaker-registry(40) 与 translation(50) **之间**——串行执行下它跑在 asr-ocr-correct(35) 之后，正好能消费 corrected segments（见 §6.2）。
...
def build_visual_context_command(request) -> list[str]:
    ...
    # 即 speaker-corrected → corrected → 原始 的优先级——与 translation 实际消费的同一份。
    # 用原始 segments.zh.json 会在 +ocr-subs 模板下与 translation 看到的时间轴/文本错位。
...
### 6.5 translation 消费（最小侵入）——匹配协议：时间重叠，不是 id 对齐
...
- **为什么不用 unit_id 匹配**（v1 方案的缺陷）：translation 的 `unit-{N}` 是运行时现场编号，visual-context 在另一个进程、可能在另一份 segments 上分组；要求两边产出相同编号 = 要求分组参数与输入文件永远逐字节一致，任何一边调参就静默错位。时间重叠匹配对两边的分组方式完全不敏感。
- 仅 `deepseek` 后端注入（m2m100 是 seq2seq 模型，不吃自然语言上下文）。
- 文件缺失/单元未命中 → 静默跳过，**翻译阶段不因视觉模块失败而失败**。
- translation 的 cache key（`_stage_cache_payload` 的 translation 分支）加一项 `"visual_context": _file_fingerprint(visual_context_path(request))`：文件不存在时 `_file_fingerprint` 返回 `exists: False` 的稳定值，所以基础模板（无 visual 节点）的 translation cache key 也会变一次——**这是一次性的全量重算成本，发版说明里写明**；之后有/无视觉上下文、视觉产物内容变化都正确触发重算。
...
uv run translip analyze-video \
  ...
  --segments output/transcription/xxx/segments.zh.json \
...
| 缓存回归 | 改 `vision_frames_per_unit` → visual-context 重算且 translation 级联重算；不改 → 全 hit |
...
### Phase 2 — 管线节点 + translation 注入（2 天）✅ 已完成（2026-06-11）
...
实测（60s 测试片，`asr-dub+visual` 模板跑到 translation）：

- 端到端：separation→transcription→speaker-registry→visual-context→translation 全部 succeeded；visual_context.json 6 单元、segments 驱动（每单元带 `segment_ids`）、场景描述质量与 Phase 0/1 一致。
- 缓存：原参数重跑 → 5 节点全部 cached；`vision_frames_per_unit: 4→2` → visual-context 重算且 translation 正确级联重算（指纹联动生效），上游 speaker-registry 之前保持 cached。
