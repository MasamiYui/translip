...
def build_task_c_command(request):
    return [
        *_cli_prefix(), "translate-script",                          # = python -m translip translate-script
        "--segments",    str(effective_task_a_segments_path(request)),  # 读：上游 transcription 的产物
        "--profiles",    str(task_b_profiles_path(request)),            # 读：上游 speaker-registry 的产物
        "--output-dir",  str(request.output_root / "translation"),           # 写：自己的产物
...
├── separation/<stem>/{voice.wav, background.wav, manifest.json}
├── transcription/voice/{segments.zh.json, transcription-manifest.json}
├── speaker-registry/voice/{speaker_profiles.json, …, speaker-registry-manifest.json}
├── translation/voice/{translation.<lang>.json, translation-manifest.json}
├── synthesis/voice/<speaker_id>/…
├── render/voice/{dub_voice.<lang>.wav, …, render-manifest.json}
...
# translation 的缓存 payload（节选）：自己的参数 + 上游的指纹一起进哈希
payload = {
    "translation_backend": request.translation_backend,         # 自己的开关
    "condense_mode":       request.condense_mode,
    "segments":  _file_fingerprint(effective_task_a_segments_path(request)),  # 上游 transcription 指纹
    "profiles":  _file_fingerprint(task_b_profiles_path(request)),            # 上游 speaker-registry 指纹
}
...
*把翻译后端从 local-m2m100 换成 deepseek：只有 translation 的缓存键变了。它的新产物 translation.json 内容指纹随之改变，于是 synthesis、render 的键也跟着失效、自动重算；而 separation / transcription / speaker-registry 的键纹丝没动，整段命中、直接复用。*

所以你只要：

```bash
# 改一个后端开关，重跑——编排器自己算出"只有 translation 往下要重算"
uv run translip run-pipeline --input ./drama-ep01.mp4 \
  --translation-backend deepseek --reuse-existing
```

编排器会复用 separation / transcription / speaker-registry 的旧产物，**只重算 translation 及其下游**。改一个 flag 精准地只动它影响到的那几站——这正是"改个 backend 就触发选择性重算"背后的机制。（想强制重算某一站，再加 `--force-stage task-x` 即可。）
...
**第三层，把它解析成一份可执行的计划。** `graph.py` 先**递归收集传递依赖**（你选了 render，它替你把 separation/a/c/d 都拉进来），再做一次**拓扑排序**，同层用 `sequence_hint` 当小顶堆的键定先后：
...
**进度是加权的。** 各阶段轻重差很多——TTS（synthesis）是最重的一站，分离、转写则轻得多。所以整体进度不是"跑完几站除以总站数"，而是**按权重加权求和**（权重在 `stages.py`）：

| 阶段 | separation | transcription | speaker-registry | translation | synthesis | render |
| --- | --- | --- | --- | --- | --- | --- |
...
这个加权进度，连同每一站的状态，被**心跳式地写进 `pipeline-status.json`**——前面几篇你在管理界面上看到的那条进度条、那个"当前在跑 synthesis"，读的就是这份文件：

```json
{ "status": "running", "overall_progress_percent": 62.5, "current_stage": "synthesis",
  "stages": [ {"stage_name": "separation", "status": "succeeded", "progress_percent": 100},
              {"stage_name": "synthesis", "status": "running",   "progress_percent": 40} ] }
```
