...
import { configApi, narratorVoicePreviewUrl, bgmPresetPreviewUrl, systemApi } from '../api/config'
import type { NarratorVoiceInfo, BgmPresetInfo } from '../api/config'
...
export function NarratorVoicePicker({
  ...
  const isViralRecap = (voice: NarratorVoiceInfo) =>
    voice.id.startsWith('narrator-recap-')
...
export function BgmPresetPicker({
  value,
  presets,
  locale,
  onChange,
...
export function NewTaskPage() {
  ...
  const { data: bgmPresets } = useQuery({
      ...
      <div className="space-y-5">
        {isCommentary ? (
          <SectionCard title={locale === 'zh-CN' ? '解说设置' : 'Commentary'} minimal>
            ...
            <Field
              label={locale === 'zh-CN' ? '解说 BGM' : 'Commentary BGM'}
              hint={
                locale === 'zh-CN'
                  ? '在 OST=0 旁白片段下铺一层背景音乐，解说说话时自动 ducking（侧链压缩）；CC0 占位音轨仅供本地学习。'
                  : 'Lays a music bed under OST=0 narration with auto-sidechain ducking. CC0 placeholder loops — local learning only.'
              }
            >
              <BgmPresetPicker
                value={config.commentary_bgm_preset ?? ''}
                presets={bgmPresets}
                locale={locale}
                onChange={value => patchConfig({ commentary_bgm_preset: value })}
              />
            </Field>
            <Field
