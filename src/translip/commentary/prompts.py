"""Prompt templates for the commentary-script chain (Phase-1 style customization).
...
Phase-1 enrichment: the writer / planner now consume tone / pacing / perspective
/ audience knobs from :mod:`translip.commentary.style_profiles`. The structure
of the chain is unchanged — only the prompt wording, density and persona vary.
"""
...
def _intensity_band(value: float) -> str:
    """Map 0.0..1.0 to a coarse band the LLM can follow."""
    if value < 0.34:
        return "轻度"
    if value < 0.67:
        return "中等"
...
def _style_brief(options: CommentaryOptions) -> str:
    """Compact "style brief" block used by planning + writing stages."""
    mode = resolve_mode(options.style)
    tone = resolve_tone(options.tone_preset)
    pacing = resolve_pacing(options.pacing_preset)
    perspective = resolve_perspective(options.perspective)
    audience = resolve_audience(options.audience)
    intensity = _intensity_band(options.style_intensity)
    cps = chars_per_second(pacing, options.language)

    lines = [
        f"叙述模式：{mode.label_zh}（{mode.summary}）",
        f"语气人格：{tone.label_zh} —— {tone.voice_brief}（风格强度：{intensity}）",
        f"节奏密度：{pacing.label_zh}（平均每段约 {pacing.avg_segment_sec:.0f}s，约 {cps:.1f} 字/秒，句式：{pacing.sentence_style}）",
        f"叙事人称：{perspective.label_zh} —— {perspective.instruction}",
        f"受众/平台：{audience.label_zh} —— {audience.language_brief}",
    ]
    if tone.do_examples:
        lines.append("语气示例可使用：" + "、".join(tone.do_examples))
    if tone.avoid:
        lines.append("语气示例避免使用：" + "、".join(tone.avoid))
    if audience.allow_phrases:
        lines.append("平台允许的高频短语（可酌情使用）：" + "、".join(audience.allow_phrases))
    if audience.banned_phrases:
        lines.append("平台禁止使用的短语：" + "、".join(audience.banned_phrases))
    return "\n".join(lines)
...
def plot_analysis(story_text: str, options: CommentaryOptions) -> dict[str, str]:
    ...
    user = (
        ...
        "为后续生成高质量影视解说脚本做准备。\n\n"
        f"# 本次解说定位\n模式：{mode.label_zh}（{mode.summary}）\n结构提示：{mode.structure_hint}\n\n"
        "# 铁律\n"
...
def segment_planning(story_text: str, plot: str, options: CommentaryOptions) -> dict[str, str]:
    ...
    user = (
        ...
        f"# 本次解说风格\n{_style_brief(options)}\n\n"
        f"# 规划焦点（来自模式 {mode.label_zh}）\n{mode.plan_focus}\n\n"
        f"# 目标原片占比\n约 {options.original_sound_ratio}%（OST=1 原声片段的总时长占比，按各片段时长估算）\n\n"
        f"# 节奏目标\n平均每段约 {pacing.avg_segment_sec:.0f} 秒；允许的最长留白 {pacing.silence_tolerance_sec:.1f} 秒。\n\n"
        "# 剧情理解材料\n" + plot + "\n\n"
        ...
        f"8. 平台风格：{audience.language_brief}\n\n"
        + _PLAN_OUTPUT_SPEC
    )
    return {"system": system, "user": user}
...
def script_generation(
    ...
    sentence_rule = {
        "short": "优先短句（多数句子 ≤ 18 字），少用从句",
        "natural": "短长句搭配，避免拖沓",
        "long": "允许较长复合句以提高信息密度，但仍要朗读顺口",
    }.get(pacing.sentence_style, "短长句搭配，避免拖沓")

    user = (
        f"# 任务\n为已规划好的片段补写解说文案。解说台词语言：{options.language}。"
        f"影视类型：{options.genre}（叙事重点：{_genre_focus(options.genre)}）。\n\n"
        f"# 本次解说风格\n{_style_brief(options)}\n\n"
        f"# 模式要点（{mode.label_zh}）\n{mode.writing_focus}\n\n"
        f"# 写作人称\n{perspective.instruction}\n示例：{perspective.example}\n\n"
        f"# 语气示范\n人格：{tone.label_zh} —— {tone.voice_brief}\n"
        + ("务必表现：" + "、".join(tone.do_examples) + "\n" if tone.do_examples else "")
        + ("务必避免：" + "、".join(tone.avoid) + "\n" if tone.avoid else "")
        ...
        f"6. 解说密度服从画面时长：按“narration 字数 / {cps:.1f} ≈ 片段秒数”估算；片段短就少说、不要灌信息。{sentence_rule}。\n"
        "7. picture 简述该片段画面中人物、动作、情绪与关键道具，便于后期识别素材。\n"
        f"8. 平台口吻：{audience.language_brief}"
        + (f"；允许短语：{', '.join(audience.allow_phrases)}" if audience.allow_phrases else "")
        + (f"；禁止短语：{', '.join(audience.banned_phrases)}" if audience.banned_phrases else "")
        + "。\n"
        "9. 不要解释规则，不要输出 Markdown 或代码块。\n\n"
        + _SCRIPT_OUTPUT_SPEC
    )
    return {"system": system, "user": user}


__all__ = ["plot_analysis", "script_generation", "segment_planning"]
