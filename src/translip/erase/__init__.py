...
In-tree port of video-subtitle-remover's inpainting core (Apache-2.0): three
backends — STTN video inpainting (default), big-LaMa single-frame inpainting,
and an OpenCV Telea fallback — driven by translip's own OCR ``detection.json``.
This makes subtitle erasure fully in-tree with no external sibling project.
...
Public API:
    EraseService().erase(video_path=..., detection_path=..., output_path=...,
                         backend="sttn"|"lama"|"opencv", ...) -> EraseResult
"""
